    ...
    if (m_currentArg == QLatin1String(TEST_OPTION)) {
        if (nextToken(RequiredToken)) {
            if (m_currentArg == QLatin1String("all")) {
                ...
                std::vector<PluginManagerPrivate::TestSpec> testSpecs;
                QVector<PluginSpec *> queue = m_pmPrivate->loadQueue();
                foreach(PluginSpec *spec, queue){
                    testSpecs.push_back(PluginManagerPrivate::TestSpec(spec));
                }
                m_pmPrivate->testSpecs = testSpecs;
